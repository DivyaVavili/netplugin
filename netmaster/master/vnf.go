...
func CreateVNF(stateDriver core.StateDriver, vnfCfg *intent.ConfigVNF) error {

	log.Infof("Received create VNF config {%v}", vnfCfg)

	/*
		var providersPresent bool
		serviceIP := vnfCfg.IPAddress

		//Check if VNF already exists.
		vnfID := GetVnfID(vnfCfg.VnfName, vnfCfg.Tenant)

		//New Service
		serviceLbState := &mastercfg.CfgServiceLBState{}
		serviceLbState.ServiceName = vnfCfg.ServiceName
		serviceLbState.Tenant = vnfCfg.Tenant
		serviceLbState.Network = vnfCfg.Network
		serviceLbState.StateDriver = stateDriver
		serviceLbState.ID = GetServiceID(vnfCfg.ServiceName, vnfCfg.Tenant)
		serviceLbState.Ports = append(serviceLbState.Ports, vnfCfg.Ports...)
		serviceLbState.Selectors = make(map[string]string)
		serviceLbState.Providers = make(map[string]*mastercfg.Provider)
		for k, v := range vnfCfg.Selectors {
		...
		if err != nil {
			log.Errorf("network %s on tenant %s is not created %s", serviceLbState.Network, vnfCfg.Tenant, networkID)
			return err
		}
...
//GetVnfID returns VNF ID for etcd lookup
func GetVnfID(vnfName string, tenantName string) string {
	return vnfName + ":" + tenantName
}
